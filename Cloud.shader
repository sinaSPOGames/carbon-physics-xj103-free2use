...
*/
shader_type spatial;
render_mode  cull_front, shadows_disabled, vertex_lighting, unshaded;

const float noise_scale = 0.00008;

uniform float seed = -1000.0;
uniform vec2 speed = vec2( 2.0, 0.0 );
uniform vec2 transform_speed = vec2( 0.0003, 0.0004 );
uniform float min_density = 0.46;
uniform float max_density = 6.0;
uniform float altitude = 2000.0;
uniform vec4 color : hint_color = vec4( 1.0, 1.0, 1.0, 1.0 );
uniform bool detail_noise = true;

uniform bool upper_enabled = true;
uniform bool lower_enabled = true;
...
void fragment( )
{
	vec3 camera_pos = ( CAMERA_MATRIX * vec4( 0.0, 0.0, 0.0, 0.0 ) ).xyz;
	// カメラ -> 描画する点への単位ベクトル
	vec3 v = -( vec4( VIEW, 10.0 ) * INV_CAMERA_MATRIX ).xyz;
	...
	ALPHA = min( abs( sin_theta ) * 10.0, 1.0 ) * clamp( ( y - min_density ) * ( max_density - min_density ), 0.0, 1.0 );
	ALBEDO = color.rgb;
	DEPTH = 1.0;
}
